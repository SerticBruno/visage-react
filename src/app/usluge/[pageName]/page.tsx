...
export async function generateMetadata({ params }: { params: { pageName: string } }): Promise<Metadata> {
  const service = services[params.pageName as keyof typeof services];
  
  if (!service) {
    return {
      title: 'Usluga nije pronađena | VISAGE studio',
      description: 'Tražena usluga nije pronađena u našoj ponudi.',
    };
  }

  return {
    title: `${service.title} | VISAGE studio - Estetski studio Sisak`,
...
export default function ServicePage({ params }: { params: { pageName: string } }) {
  const service = services[params.pageName as keyof typeof services];
